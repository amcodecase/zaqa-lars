...
//                $cellValue = $worksheet->getCell($cellCoordinate)->getCalculatedValue();
            ...
            // Add institution_id to row data
            $rowData[] = $institutionId;
            $columnsWithInstitution = array_merge($expectedColumns, ['institution_id']);

            // Validate row data
            $validation = validateRowData($rowData, $columnsWithInstitution, $dataType, $row);
            if (!$validation['valid']) {
            ...
            $duplicateCheck = checkForDuplicates($pdo, $tableName, $rowData, $columnsWithInstitution, $dataType);
            ...
            // Insert record
            if (insertRecord($pdo, $tableName, $rowData, $columnsWithInstitution)) {
                $successCount++;
            } else {
                $errorCount++;
...
function validateRowData($rowData, $expectedColumns, $dataType, $rowNumber) {
    $errors = [];
...
//    // Gender validation
//    if (empty($columnMapping['gender']) || !in_array(strtolower($columnMapping['gender']), ['male', 'female'])) {
...
//
//    if ($yearAwarded === '' || !ctype_digit($yearAwarded)) {
//        $errors[] = 'Year Awarded is required and must be a 4-digit numeric value';
//    } else {
//        $yearAwarded = (int)$yearAwarded;
//        if ($yearAwarded < 1000 || $yearAwarded > 9999) {
//            $errors[] = 'Year Awarded must be a valid 4-digit year';
//        }
//    }
...
function checkForDuplicates($pdo, $tableName, $rowData, $expectedColumns, $dataType) {
    $columnMapping = array_combine($expectedColumns, $rowData);
...
function insertRecord($pdo, $tableName, $rowData, $expectedColumns) {
    try {
        $columnMapping = array_combine($expectedColumns, $rowData);

        // Normalize gender value
        if (isset($columnMapping['gender'])) {
            $columnMapping['gender'] = ucfirst(strtolower($columnMapping['gender']));
        }

        // Add added_by field
        $columns = array_merge($expectedColumns, ['added_by']);
        $values = array_merge(array_values($columnMapping), [$_SESSION['user_id']]);

        // Handle null values for optional fields
        $optionalFields = ['other_names', 'nrc_number', 'passport_no', 'level', 'trade_code', 'sub_institution'];
        $processedValues = [];

        foreach ($values as $index => $value) {
            if ($value === '' && in_array($columns[$index], $optionalFields)) {
        ...
        return $stmt->execute($processedValues);
...
function getInstitutions($pdo) {
    try {
        ...
        // Debug: Log the institutions being returned
        error_log("Fetched institutions: " . json_encode($institutions));
...
<head>
    ...
    <link rel="stylesheet" href="../../../../assets/css/admin-bulk.css">
