// TEVETA Records JavaScript functionality
...
function takeScreenshot(button) {
    ...
    html2canvas(recordCard, options).then(function(canvas) {
        ...
        const candidateIdElement = recordCard.querySelector('.detail-row:nth-child(1) .value');

        let filename = 'teveta-record';
        if (recordId) {
            filename += `-${recordId}`;
        }
        if (candidateIdElement) {
            const candidateId = candidateIdElement.textContent.trim();
